...
describe("Vault", function() {
    ...
    context("lockTokens", async () => {
        it("creates valid lock of Arch tokens", async function() {
            ...
            await vault.lockTokens(archToken.address, deployer.address, alice.address, START_TIME, lockAmount, DURATION_IN_DAYS, DURATION_IN_DAYS, true)
        ...
        it("does not allow a lock with a duration of 0", async function() {
            ...
            let totalLocked = await archToken.balanceOf(vault.address)
            let lockAmount = ethers.BigNumber.from(1000).mul(ethers.BigNumber.from(10).pow(decimals))
            await expect(vault.lockTokens(archToken.address, deployer.address, bob.address, START_TIME, lockAmount, DURATION_IN_DAYS, DURATION_IN_DAYS, false)).to.revertedWith("revert Vault::lockTokens: vesting duration must be > 0")
            expect(await archToken.balanceOf(vault.address)).to.eq(totalLocked)
        ...
        it("does not allow a lock with a duration of > 25 years", async function() {
            ...
            let totalLocked = await archToken.balanceOf(vault.address)
            let lockAmount = ethers.BigNumber.from(1000).mul(ethers.BigNumber.from(10).pow(decimals))
            await expect(vault.lockTokens(archToken.address, deployer.address, bob.address, START_TIME, lockAmount, DURATION_IN_DAYS, DURATION_IN_DAYS, false)).to.revertedWith("revert Vault::lockTokens: vesting duration more than 25 years")
            expect(await archToken.balanceOf(vault.address)).to.eq(totalLocked)
            const emptyLocks = await vault.activeLocks(bob.address)
            expect(emptyLocks.length).to.eq(0)
        })
        ...
        it("does not allow a lock of 0", async function() {
            ...
            await expect(vault.lockTokens(archToken.address, deployer.address, bob.address, START_TIME, lockAmount, DURATION_IN_DAYS, DURATION_IN_DAYS, false)).to.revertedWith("revert Vault::lockTokens: amount not > 0")
            expect(await archToken.balanceOf(vault.address)).to.eq(totalLocked)
            const emptyLocks = await vault.activeLocks(bob.address)
            expect(emptyLocks.length).to.eq(0)
        })
        ...
        it("does not allow a lock when locker has insufficient balance", async function() {
            ...
            let totalLocked = await archToken.balanceOf(vault.address)
            let lockAmount = ethers.BigNumber.from(1000).mul(ethers.BigNumber.from(10).pow(decimals))
            await expect(vault.lockTokens(archToken.address, deployer.address, bob.address, START_TIME, lockAmount, DURATION_IN_DAYS, DURATION_IN_DAYS, false)).to.revertedWith("revert Arch::_transferTokens: transfer exceeds from balance")
            expect(await archToken.balanceOf(vault.address)).to.eq(totalLocked)
            const emptyLocks = await vault.activeLocks(bob.address)
            expect(emptyLocks.length).to.eq(0)
        })
    ...
    context("lockTokensWithPermit", async () => {
        xit("creates valid lock of Arch tokens", async function() {
            let decimals = await archToken.decimals()
            const START_TIME = parseInt(Date.now() / 1000) + 21600
            const DURATION_IN_DAYS = 4
            let totalLocked = await archToken.balanceOf(vault.address)
            ...
            await vault.lockTokensWithPermit(archToken.address, deployer.address, alice.address, START_TIME, lockAmount, DURATION_IN_DAYS, 0, false, deadline, v, r, s)
            const activeLocks = await vault.activeLocks(alice.address)
            const newLock = activeLocks[0]
            expect(newLock[0]).to.eq(archToken.address)
            expect(newLock[1]).to.eq(alice.address)
        ...
        xit("does not allow a lock with a duration of 0", async function() {
            ...
            await expect(vault.lockTokensWithPermit(archToken.address, deployer.address, alice.address, START_TIME, lockAmount, DURATION_IN_DAYS, 0, false, deadline, v, r, s)).to.revertedWith("revert Vault::lockTokensWithPermit: vesting duration must be > 0")
            expect(await archToken.balanceOf(vault.address)).to.eq(totalLocked)
            const emptyLocks = await vault.activeLocks(bob.address)
            expect(emptyLocks.length).to.eq(0)
        })
        ...
        xit("does not allow a lock with a duration of > 25 years", async function() {
            ...
            await expect(vault.lockTokensWithPermit(archToken.address, deployer.address, alice.address, START_TIME, lockAmount, DURATION_IN_DAYS, 0, false, deadline, v, r, s)).to.revertedWith("revert Vault::lockTokensWithPermit: vesting duration more than 25 years")
            expect(await archToken.balanceOf(vault.address)).to.eq(totalLocked)
            const emptyLocks = await vault.activeLocks(bob.address)
            expect(emptyLocks.length).to.eq(0)
        })
        ...
        xit("does not allow a lock of 0", async function() {
            ...
            const { v, r, s } = ecsign(Buffer.from(digest.slice(2), 'hex'), Buffer.from(DEPLOYER_PRIVATE_KEY, 'hex'))
            await expect(vault.lockTokensWithPermit(archToken.address, deployer.address, alice.address, START_TIME, lockAmount, DURATION_IN_DAYS, 0, false, deadline, v, r, s)).to.revertedWith("revert Vault::lockTokensWithPermit: amount not > 0")
            expect(await archToken.balanceOf(vault.address)).to.eq(totalLocked)
            const emptyLocks = await vault.activeLocks(bob.address)
            expect(emptyLocks.length).to.eq(0)
        })
        ...
        xit("does not allow a lock when locker has insufficient balance", async function() {
            ...
            await archToken.transfer(bob.address, await archToken.balanceOf(deployer.address))
            await expect(vault.lockTokensWithPermit(archToken.address, deployer.address, alice.address, START_TIME, lockAmount, DURATION_IN_DAYS, 0, false, deadline, v, r, s)).to.revertedWith("revert Arch::_transferTokens: transfer exceeds from balance")
            expect(await archToken.balanceOf(vault.address)).to.eq(totalLocked)
            const emptyLocks = await vault.activeLocks(bob.address)
            expect(emptyLocks.length).to.eq(0)
        })
    ...
    context("tokenBalance", async () => {
        it("returns 0 if locked token balance does not exist", async function() {
          ...
          const DURATION_IN_DAYS = 4
          let lockAmount = ethers.BigNumber.from(1000).mul(ethers.BigNumber.from(10).pow(decimals))
          await vault.lockTokens(archToken.address, deployer.address, bob.address, START_TIME, lockAmount, DURATION_IN_DAYS, 0, false)
        ...
        it("returns total as claimable amount after lock duration has ended", async function() {
          ...
          const balance = await vault.tokenBalance(archToken.address, bob.address)
        ...
        it("returns correct locked tokens for balances at different stages (multiple)", async function() {
            ...
            const DURATION_IN_SECS = DURATION_IN_DAYS * 24 * 60 * 60
            let lockAmount = ethers.BigNumber.from(1000).mul(ethers.BigNumber.from(10).pow(decimals))
            await vault.lockTokens(archToken.address, deployer.address, bob.address, START_TIME, lockAmount, DURATION_IN_DAYS, DURATION_IN_DAYS, false)
            await vault.lockTokens(archToken.address, deployer.address, bob.address, START_TIME, lockAmount, DURATION_IN_DAYS * 2, DURATION_IN_DAYS * 2, false)
            await ethers.provider.send("evm_setNextBlockTimestamp", [START_TIME + DURATION_IN_SECS])
            await ethers.provider.send("evm_mine")
            const balance = await vault.tokenBalance(archToken.address, bob.address)
            expect(balance.totalAmount).to.eq(lockAmount.mul(2))
    ...
    context("claimableBalance", async () => {
      ...
      it("returns 0 if cliff has yet to pass", async function() {
        ...
        let lockAmount = ethers.BigNumber.from(1000).mul(ethers.BigNumber.from(10).pow(decimals))
        await vault.lockTokens(archToken.address, deployer.address, bob.address, START_TIME, lockAmount, DURATION_IN_DAYS, CLIFF_DURATION_IN_DAYS, false)
    ...
    context("claimUnlockedTokenAmounts", async () => {
        ...
        it("allows user to claim unlocked tokens once", async function() {
            ...
            const DURATION_IN_SECS = DURATION_IN_DAYS * 24 * 60 * 60
            let lockAmount = ethers.BigNumber.from(1000).mul(ethers.BigNumber.from(10).pow(decimals))
            await vault.lockTokens(archToken.address, deployer.address, alice.address, START_TIME, lockAmount, DURATION_IN_DAYS, 0, false)
        ...
        it("allows user to claim unlocked tokens multiple times", async function() {
            ...
            expect(await vault.claimableBalance(0)).to.eq(0)
            expect(await archToken.balanceOf(alice.address)).to.eq(aliceBalanceBefore.add(lockAmount))
    ...
    context("extendLock", async () => {
        it("allows receiver to extend a lock", async function() {
            ...
            let lockAmount = ethers.BigNumber.from(1000).mul(ethers.BigNumber.from(10).pow(decimals))
            await vault.lockTokens(archToken.address, deployer.address, bob.address, START_TIME, lockAmount, ORIGINAL_DURATION_IN_DAYS, 0, false)
